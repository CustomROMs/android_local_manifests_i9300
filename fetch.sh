...
fetch() {
	...
	while read line; do
		...
		echo
		echo "git -C $ANDROID_BUILD_TOP/$proj fetch https://github.com/CustomROMS/$(path2name $proj) lineage-17.1"
		git -C $ANDROID_BUILD_TOP/$proj fetch https://github.com/CustomROMS/$(path2name $proj) lineage-17.1
		echo ""; echo "";
